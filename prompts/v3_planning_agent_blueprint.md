...
3. 例如“技术方法 → 核查准备工作 → 独立计分任务 → 满分原子条件”应形成四级
   结构，并用输入中的 condition_id 逐项绑定，不能仅靠相似标题声称覆盖。
4. 每个 `condition_id` 只能声明一次，并且必须位于其所属
    `ScoreResponseUnit` 的 primary 节点子树中。`content`、`evidence`、以及带
    明确业务写作对象的 `quality` 条件要形成可写、可检查的章节绑定。同一 Unit 内
    每个原子 `content`/`evidence` 或有明确主题的 `quality` 条件必须绑定不同的可
    检查节点，不能把多项实质满分条件塞回一个泛化评分标题。项目任务背景、工作
    必要性与可行性、工作目标、工作内容、数据接收、检查方法等均为明确主题，即使
    原文同时有“清楚/充分/可行”等质量措辞也要单独成节。只有“完整性”“合理性”
    “可行性”等没有具体对象的纯质量约束，才绑定该 Unit 的 primary 节点并转为
    writing objectives；不得生成这些空洞质量章节。
5. 除评分组根章节外，必须沿输入的 `outline_path` 保留来源语义层级：先去掉与
   评分组标题重复的首段，再复用同组公共路径节点。最后一个路径节点或独立任务节点
   绑定唯一 `primary_response_unit_ids`；条件节点继续作为其子树。不得为了压缩层级
   丢弃路径；确有嵌套时允许第五级或更深层级。
