...
4. 每个 `condition_id` 只能声明一次，并且必须位于其所属
    ...
    检查节点，不能把多项实质满分条件塞回一个泛化评分标题。严禁将同一
    ScoreResponseUnit 下的多个 condition_id 全部写入同一个章节节点的
    `score_condition_ids` 字段——每个 condition_id 必须独占一个不同的可见节点。
    项目任务背景、工作必要性与可行性、工作目标、工作内容、数据接收、检查方法等
    均为明确主题，即使原文同时有“清楚/充分/可行”等质量措辞也要单独成节。只有
    “完整性”“合理性”“可行性”等没有具体对象的纯质量约束，才绑定该 Unit 的
    primary 节点并转为 writing objectives；不得生成这些空洞质量章节。
