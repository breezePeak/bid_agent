...
from .scoring_outline_policy import outline_structure_key
...
def _path_for_unit(unit: object, point: object, group_title: str) -> list[str]:
    ...
    for item in path:
        if (
            not compact
            or outline_structure_key(compact[-1])
            != outline_structure_key(item)
        ):
            compact.append(item)
    return compact
    ...
    for group in scores.groups:
        ...
        for point, unit in units:
            ...
            for path_index, title in enumerate(path):
                key_parts.append(outline_structure_key(title) or title)
                key = tuple(key_parts)
            ...
            nodes[primary_index] = nodes[primary_index].model_copy(
                update={
                    ...
                    "primary_response_unit_ids": list(
                        dict.fromkeys(
                            [*primary.primary_response_unit_ids, unit.unit_id]
                        )
                    ),
                    "score_condition_ids": list(
                        dict.fromkeys(
                            [*primary.score_condition_ids, *unit_condition_ids]
                        )
                    ),
                    ...
                    "planned_tables": list(
                        dict.fromkeys(
                            [
                                *primary.planned_tables,
                                *(
                                    ["证明材料清单"]
                                    if any(
                                        conditions_by_id[condition_id].condition_role
                                        == "evidence"
                                        for condition_id in unit_condition_ids
                                    )
                                    else []
                    ...
                    "confidence": min(primary.confidence, unit.confidence),
                }
            )
