...
class ChapterWorkspaceRecord(BaseModel):
    ...
    head_plan_revision: int = Field(default=0, ge=0)
    confirmed_plan_revision: int = Field(default=0, ge=0)
    plan_status: Literal[
        "not_started",
        "current",
        "confirmed",
...
class ChapterPlanStatus(str, Enum):
    ...
    CURRENT = "current"
    CONFIRMED = "confirmed"
    STALE_BLUEPRINT = "stale_blueprint"
    STALE_GLOBAL_CONTEXT = "stale_global_context"
    STALE_CHAPTER_CONTEXT = "stale_chapter_context"
    STALE_SOURCE = "stale_source"
    STALE_EVIDENCE = "stale_evidence"


class ChapterPlanContentUnit(BaseModel):
    """A deterministic, ordered unit of a chapter writing plan."""
...
class ChapterPlanBinding(BaseModel):
    ...
    blueprint_revision: int = Field(ge=1)
    blueprint_hash: str = Field(min_length=1)
    global_context_revision: int = Field(default=0, ge=0)
    global_context_hash: str = ""
    chapter_context_revision: int = Field(default=0, ge=0)
    chapter_context_hash: str = ""
    source_index_revision: int = Field(default=0, ge=0)
    source_index_hash: str = ""
    evidence_revision: int = Field(default=0, ge=0)
    evidence_hash: str = ""

    @model_validator(mode="after")
    def revision_hash_pairs_are_complete(self) -> "ChapterPlanBinding":
        for revision_name, hash_name in (
            ("global_context_revision", "global_context_hash"),
            ("chapter_context_revision", "chapter_context_hash"),
            ("source_index_revision", "source_index_hash"),
            ("evidence_revision", "evidence_hash"),
        ):
            revision = int(getattr(self, revision_name))
            digest = str(getattr(self, hash_name) or "")
            if revision > 0 and not digest:
                raise ValueError(f"{hash_name} 在 revision>0 时不能为空")
            if revision == 0 and digest:
                raise ValueError(f"{hash_name} 在 revision=0 时必须为空")
        return self


class ChapterWritingPlanCandidate(BaseModel):
    """Strict agent/user candidate; the service injects the authoritative binding."""

    model_config = ConfigDict(extra="forbid", str_strip_whitespace=True)

    schema_version: Literal["v3.chapter-writing-plan.v2"] = (
    ...
    @model_validator(mode="after")
    def content_units_are_deterministic(self) -> "ChapterWritingPlanCandidate":
        unit_ids = [item.unit_id for item in self.content_units]
        orders = [item.order for item in self.content_units]
        if len(unit_ids) != len(set(unit_ids)):
            raise ValueError("ChapterWritingPlanCandidate 不允许重复 unit_id")
        if len(orders) != len(set(orders)):
...
class ChapterWritingPlanPayload(ChapterWritingPlanCandidate):
    """Persisted plan payload with an authoritative dependency binding."""

    binding: ChapterPlanBinding


class ChapterWritingPlanRevisionRecord(BaseModel):
    model_config = ConfigDict(extra="forbid", str_strip_whitespace=True)

    chapter_id: str = Field(min_length=1)
    plan_revision: int = Field(ge=1)
    parent_plan_revision: int | None = Field(default=None, ge=1)
...
class ChapterPlanApprovalReceiptRecord(BaseModel):
    model_config = ConfigDict(extra="forbid", str_strip_whitespace=True)
