...
_STAGE_DESCRIPTIONS: dict[str, str] = {
    ...
    "select_contexts": "主 Agent 派发章节上下文选择子 Agent，并发生成上下文且支持显式断点续跑。",
    ...
    return ToolSpec(
        ...
        params_schema={
            ...
            "properties": {
                ...
                "resume": {"type": "boolean", "default": True, "description": "复用有效检查点（上下文选择阶段）"},
                "workers": {"type": "integer", "minimum": 1, "description": "并发 worker（章节类阶段）"},
                "max_retries": {"type": "integer", "minimum": 0},
                "chapter_ids": {"type": "array", "description": "审核改稿的定向章节范围"},
            },
    ...
    return ToolSpec(
        ...
        params_schema={
            ...
            "properties": {
                ...
                "chapter_ids": {
                    "type": "array",
                    "description": "仅执行指定章节；章节写作和审核改稿阶段支持。",
                },
            },
            "additionalProperties": False,
        },
