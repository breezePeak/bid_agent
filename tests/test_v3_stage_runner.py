...
class V3StageRunnerTests(unittest.TestCase):
 def test_runs_v3_content_chain_and_rejects_unknown_stage(self):
  with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as t:
   ...
   for stage in ('sync_material_requirements','compile_document_contract','plan_document','execute_content_plan','integrate_document'):r.run(stage)
 ...
 def test_render_is_refused_until_the_content_gate_passes(self):
  with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as t:
   ...
   for stage in ('compile_document_contract','plan_document','execute_content_plan','integrate_document'):r.run(stage)
