...
class V3StageRunnerTests(unittest.TestCase):
 def test_runs_v3_content_chain_and_rejects_unknown_stage(self):
  with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as t:
   ...
   for stage in ('ingest_inputs','normalize_sources','build_requirement_ledger','build_project_model','sync_material_requirements','compile_document_contract','plan_document','execute_content_plan','integrate_document'):r.run(stage)
