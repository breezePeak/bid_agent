...
V3 立项前记录的一次历史异常运行中（仅作为旧链路问题证据，不是人工 Golden、业务规模或发布阈值）：
...
这里的 `92` 未经人工 Golden 证明为正确评分点数量，`198` 也只是旧链路创建的任务数，不能改写为“真实模板有 198 个 canonical 节点”。二者只能用于说明重复绑定和按节点线性扇出的历史缺陷。
...
14. 运行时 Quality Gate 通过只代表单项目满足机器门禁；产品版本未通过独立真实项目盲测 Gate U 前，Renderer 输出只能标记为测试草稿，不能宣称正式标书或切换生产写路径。
...
图中的 `Quality Gate Set` 属于工作空间运行时 G0～G6/H1。仓库发布 Gate U 不参与单份标书的 Artifact 晋级，而是在 PR-27 生产切换前使用独立 holdout、专家盲审、人工改写量和最终 Word 逐页检查，证明声明的 supported profile 确实可用；Gate M 必须依赖已通过的 Gate U。
