...
| V2.0-B109 | 2026-07-22 | MaterialSubmission 写入前必须复核当前工作区已消费 upload_token 的文件名、hash 与大小；内部调用方也不能伪造提交历史或把其他上传证据绑定到材料，状态/证据不一致一律拒绝。 |
...
| V2.0-B135 | 2026-07-22 | 修复工作区切换时问题面板尝试写入只读计算值的前端错误；现在只重置本地材料徽标缓存，V2 Snapshot 的权威材料状态继续由状态总线提供。 |
