...
以下内容是后续 V3 必须保留的历史回归场景说明，不把旧链路输出当作正确结果。只有补齐原始输入、人工预期、输入 hash 和版本信息后，历史场景才能注册为可执行 Golden fixture：

1. 深层模板历史异常：旧 `plan_chapter_jobs` 曾创建 198 个独立 Writer 任务；现有记录不能据此断言原模板具有 198 个 canonical 节点。
2. 评分点重复历史异常：一次旧链路运行识别出 92 个评分点，却累计产生 1198 次绑定，其中旧运行 ID `S036` 曾被绑定 131 次；这些数值只描述旧缺陷，不是固定验收规模。
