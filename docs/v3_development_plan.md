...
- 一次历史异常运行曾产生 198 个独立章节任务和 1198 次评分点绑定，证明旧 job 模型不能继续沿用；这些数值不是现行 Golden 或业务规模。
...
- Writer/Agent 数量不得随模板节点或章节数量线性增长；历史 198 个独立任务异常只作为回归背景。
- 历史热点条目（旧运行 ID `S036`）不得再次出现无法解释的 131 次重复主责绑定。
...
评分语义和模板结构分开验收，不把历史 `92/198` 统计拼成一个固定样本：

- 每份真实样本的评分点数量以原始文件和双人复核的人工 Golden 为准；
- 每个 Golden 原子评分点有且只有一个受控主责 Duty/章节；
- 任意模板规模下，Writer/Agent 数量都不得与模板节点或章节数量线性对应；
- 历史 `92/1198/S036=131` 只有在原始输入和人工预期完成冻结后才能成为可执行回归；
- 合成深层模板只验证结构保真、确定性和扇出约束，不代表真实业务模板规模；
- 模板固定标题和结构不变；
- 同一主题不在多章重复完整论述；
- 全文数字、范围、工期和交付物一致。
