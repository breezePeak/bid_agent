...
负责合并重复响应主题、按评审阅读顺序生成项目专用目录，并在每个 `ChapterNode` 上
保存主责/支撑 Unit、Condition 和 Requirement ID。评分表已有的因素父链必须复用为
唯一公共路径；末级评分因素直接承接 Unit，原子条件直接作为其子节点，不得再插入改写
出来的冗余“独立任务”层。G2 只检查客观
关系：
